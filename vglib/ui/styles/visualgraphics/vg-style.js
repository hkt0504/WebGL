...
VG.Styles.VisualGraphics=function( dontLoadImages )
{
    ...
    if ( !dontLoadImages ) {
        ...
        VG.loadStyleImage( "visualgraphics", "logo_256.png" );
        ...
        VG.loadStyleImage( "visualgraphics", "vgstyle_nodes_bg.png" );    
    }
        ...
        this.bluish= {
            ...
            DockStripWidget : {
                ...
                TextButton : {
                    ...
                    BackgroundColor : VG.Core.Color( 143, 149, 159 ),
            ...
            IconStrip : {
                ...
                SelectedItem : {
                    BackgroundColor : VG.Core.Color( "#6064cf" ),
                    BorderColor : VG.Core.Color( "#6064cf" ),
                    TextColor : VG.Core.Color( 255, 255, 255 ),
                },
                Item : {
                    TextColor : VG.Core.Color( 255, 255, 255 ),
...
VG.Styles.VisualGraphics.prototype.drawToolButton=function( canvas, button )
{
    ...
    } else
    {
        ...
        if ( button.icon ) 
        {
            ...
            {
                if ( !button.clickedIcon || ( button.clickedIcon.stylePath !== VG.context.style.path ) )
                {
                    button.clickedIcon=VG.Core.Image( button.icon.width, button.icon.height );
                    button.clickedIcon.stylePath=VG.context.style.path;
                    for ( var h=0; h < button.icon.height; ++h )
                    {
                        for ( var w=0; w < button.icon.width; ++w )
                        {
...
VG.Styles.VisualGraphics.prototype.drawGeneralBorder=function( canvas, edit )
{
    if ( !edit.embedded )
    {
        ...
        edit.contentRect.copy( edit.rect.shrink( 1, 1, edit.contentRect ) );
...
VG.Styles.VisualGraphics.prototype.drawTreeWidgetItem=function( canvas, item, selected, rect, contentRect, indentTop )
{
    ...
    if ( !selected ) {
        if ( !item.icon || ( item.icon.stylePath !== VG.context.style.path ) && item.iconName && item.iconName.length ) 
        {
        ...
        if ( !item.selectedIcon || ( item.selectedIcon.stylePath !== VG.context.style.path ) && item.iconName && item.iconName.length ) 
        {
            item.selectedIcon=VG.context.imagePool.getImageByName( item.iconName );
            if ( item.selectedIcon ) {
                //item.selectedIcon=this.multipliedImage( item.selectedIcon, 0.52, 0.58, 0.90 );
    ...
    {
        var offset=0;
        if ( indentTop ) offset=this.skin.TreeWidget.ArrowSize.height + 8;
        ...
        if ( selected ) 
        {
            ...
            {
                var y=rect.y + (rect.height - icon.height)/2;
                canvas.drawImage( VG.Core.Point( rect.x, y ), icon );
...
VG.Styles.VisualGraphics.prototype.drawIconStripBackground=function( canvas, widget )
{
    if ( widget.visualState === VG.UI.Widget.VisualState.Focus )    
        canvas.draw2DShape( VG.Canvas.Shape2D.RoundedRectangleOutline1px, widget.rect, this.skin.TreeWidget.SelectionBorderColor );
...
VG.Styles.VisualGraphics.prototype.drawIconStripSelectedItemBackground=function( canvas, itemRect, widget )
{
