...
VG.RenderTarget = function(w, h, main)
{
	...
	// color buffer to ghost by glReadPixel when it needs.
	this.pixelBuffer = null; // Uint8Array
	this.pixelBufferSize = 0; // buffer's size
...
VG.RenderTarget.prototype.readPixelBuffer = function()
{
    /** Creates and reads the render-target's color buffer (texture). The calling usage likes as bellow.<br />
	 * [Usage]<br />
	 * // "rt" is render-target object.<br />
	 * rt.bind(); // It needs not to call if it bound before(yet).<br />
	 * image = rt.readPixelBuffer(); // The image is the {VG.Core.Image} to return.<br />
	 * rt.unbind(); // Call it only if need to call that.
	 */
	...
	if (this.pixelBufferSize < bufferSize)
	{
		this.pixelBuffer = new Uint8Array(bufferSize);
		this.pixelBufferSize = bufferSize;
	}
	
	// read pixel buffer
	gl.readPixels(0, this.getRealHeight()-height, width, height, gl.RGBA, gl.UNSIGNED_BYTE, this.pixelBuffer);
}

VG.RenderTarget.prototype.getPixelRawByte = function(index)
{
    /** Reads pixel(color rgba) from pre-read buffer by readPixelBuffer().
     *  @param {int} index - memory index of pixel's color componet(r, g, b, a) */
...
VG.RenderTarget.prototype.readPixel = function(x, y)
{
    /**
     * Read a pixel from this frame buffer
     * @param x {Number} x of pixel
     * @param y {Number} y of pixel
     * @return {Dictionary{}} contains r, g, b, a in 0-255 range each
     */
    var gl = VG.WebGL.gl;
    var ab = new ArrayBuffer(4);
    var pixels = new Uint8Array(ab);
    gl.readPixels(x, y, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
    var color = (new Uint32Array(ab))[0];
    return {r: color & 0xff, g: (color >> 8) & 0xff, b: (color >> 16) & 0xff, a: (color >> 24)};
};
